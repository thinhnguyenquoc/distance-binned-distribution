...
def train_production_model(
    ...
    assert best_state is not None, "best_state cannot be None!"
    ...
    for epoch in range(1, epochs + 1):
        ...
        for c_idx in perm_cities:
            ...
            while i < n_active:
                ...
                if not batch_orig:
                    if i < n_active:
                        batch_orig.append(shuffled_origs[i].item())
                        i += 1
                    else:
                        break
                num_b_orig = len(batch_orig)
                max_batch_pairs_observed = max(max_batch_pairs_observed, curr_pairs)
                ...
                b_compact_o = torch.cat(b_compact_list, dim=0)
                optimizer.zero_grad()
                loss = model.compute_loss_batch(b_feat, b_compact_o, b_trips, num_b_orig)
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0)
                optimizer.step()
                train_loss_weighted_sum += loss.item() * num_b_orig
                train_origins_total += num_b_orig
        model.eval()
        val_city_losses = []
        with torch.no_grad():
            for item in val_items:
                ...
                while i < n_active:
                    ...
                    city_loss_sum += b_loss * num_b_orig
                    city_origin_count += num_b_orig
                if city_origin_count > 0:
                    val_city_losses.append(city_loss_sum / city_origin_count)
        mean_val_loss = float(np.mean(val_city_losses)) if val_city_losses else float("inf")
        train_mean_loss = train_loss_weighted_sum / max(1, train_origins_total)
        ...
        if mean_val_loss < best_val_loss - 1e-5:
            ...
            best_state = copy.deepcopy(model.state_dict())
            is_best = True
        print(f"  Epoch {epoch:2d}/{epochs} | Train CE: {train_mean_loss:.4f} | Val CE: {mean_val_loss:.4f} (best: {best_val_loss:.4f} @ ep {best_epoch}) {'*' if is_best else ''} | Time: {ep_time:.1f}s")
    last_state = copy.deepcopy(model.state_dict())
    total_time = time.time() - t0
    print(f"  DG training completed in {total_time:.1f}s. Best epoch: {best_epoch} with Val CE: {best_val_loss:.4f} | Max pairs in batch: {max_batch_pairs_observed:,}")
    assert best_state is not None, "best_state cannot be None!"
    return model, best_state, last_state, best_epoch, best_val_loss, total_time, max_batch_pairs_observed
def evaluate_pilot_fold1(
    ...
    print("--- [Step 3] Evaluating on Fold 1 Test Cities via Batched CSR Inference on Omega_c ---")
    prod_model.eval()
    dg_model.eval()
    results = []
    for city_name in test_cities:
        ...
        o_idx_om = cd.pair_o_idx[omega_mask]
        d_idx_om = cd.pair_d_idx[omega_mask]
        dist_om = cd.pair_distance[omega_mask].unsqueeze(1)
        t_gt_om = cd.pair_trips[omega_mask].numpy().astype(np.float64)
        n_omega = len(o_idx_om)
        sort_perm, offsets, active_origins = build_origin_csr(o_idx_om, n_t)
        sorted_o = o_idx_om[sort_perm]
        sorted_d = d_idx_om[sort_perm]
        sorted_dist = dist_om[sort_perm]
        sorted_feat = torch.cat([cd.node_features[sorted_o], cd.node_features[sorted_d], sorted_dist], dim=1).to(device)
        with torch.no_grad():
            probs_t = dg_model.compute_probs_batched_csr(
                sorted_feat, sort_perm, offsets, active_origins, n_omega, batch_origins=64, max_pairs_per_batch=max_pairs_per_batch, device=device
            )
            probs_om = probs_t.cpu().numpy().astype(np.float64)
            p_sum = np.zeros(n_t, dtype=np.float64)
            np.add.at(p_sum, o_idx_om.numpy(), probs_om)
            act_orig_np = active_origins.numpy()
            max_prob_err = float(np.max(np.abs(p_sum[act_orig_np] - 1.0)))
            assert max_prob_err < 1e-5, f"{city_name}: Prob sum error {max_prob_err}"
            O_hat = prod_model.predict_outflow(cd.node_features.to(device)).cpu().numpy().astype(np.float64)
            t_pred_o_om = O_hat[o_idx_om.numpy()] * probs_om
            O_true_omega = np.zeros(n_t, dtype=np.float64)
            np.add.at(O_true_omega, o_idx_om.numpy(), t_gt_om)
            t_oracle_o_om = O_true_omega[o_idx_om.numpy()] * probs_om
        ...
        assert abs(np.sum(t_oracle_cal_om) - tot_oracle) < 1e-4 * max(1.0, tot_oracle)
        orig_pred_before = np.zeros(n_t, dtype=np.float64)
        orig_pred_after  = np.zeros(n_t, dtype=np.float64)
        np.add.at(orig_pred_before, o_idx_om.numpy(), t_pred_o_om)
        np.add.at(orig_pred_after,  o_idx_om.numpy(), t_pred_cal_om)
        drift_pred = np.abs(orig_pred_after[act_orig_np] - orig_pred_before[act_orig_np]) / np.maximum(orig_pred_before[act_orig_np], 1.0)
        mean_drift_pred = float(np.mean(drift_pred))
        max_drift_pred = float(np.max(drift_pred))
        orig_orc_before = np.zeros(n_t, dtype=np.float64)
        orig_orc_after  = np.zeros(n_t, dtype=np.float64)
        np.add.at(orig_orc_before, o_idx_om.numpy(), t_oracle_o_om)
        np.add.at(orig_orc_after,  o_idx_om.numpy(), t_oracle_cal_om)
        drift_orc = np.abs(orig_orc_after[act_orig_np] - orig_orc_before[act_orig_np]) / np.maximum(orig_orc_before[act_orig_np], 1.0)
        mean_drift_orc = float(np.mean(drift_orc))
        max_drift_orc = float(np.max(drift_orc))
        o_hat_act = O_hat[act_orig_np]
        o_true_act = O_true_omega[act_orig_np]
        ...
        rec = {
            ...
            "prod_r": prod_r,
            "pred_mean_orig_drift": mean_drift_pred,
            "pred_max_orig_drift": max_drift_pred,
            "oracle_mean_orig_drift": mean_drift_orc,
            "oracle_max_orig_drift": max_drift_orc,
            "prob_sum_max_error": max_prob_err,
        }
        results.append(rec)
        print(f"  {city_name:<16} | Pred: {cpc_pred_before:.4f} -> {cpc_pred_after:.4f} ({delta_pred:+.4f}, ratio {ratio_pred:.2f}, drift {mean_drift_pred:.2%}) | Oracle: {cpc_oracle_before:.4f} -> {cpc_oracle_after:.4f} ({delta_oracle:+.4f})")
    return results
def main():
    ...
    parser.add_argument("--max_pairs_per_batch", type=int, default=250000)
    ...
    manifest_real_sha256 = hashlib.sha256(manifest_bytes).hexdigest()
    
    splits = load_splits_manifest_v2(str(manifest_path), data_root=args.data_root)
    ...
    print(f"Manifest Real SHA256: {manifest_real_sha256[:12]}...")
    ...
    dg_model, best_dg_state, last_dg_state, dg_epoch, dg_val_ce, dg_time, max_b_pairs = train_deep_gravity_allocation(
        train_cities=train_cities,
        val_cities=val_cities,
        seed=args.seed,
        epochs=20,
    ...
    torch.save({
        ...
        "manifest_sha256": manifest_real_sha256,
    ...
    mean_drift_pred_all = float(np.mean([r["pred_mean_orig_drift"] for r in city_results]))
    mean_drift_orc_all  = float(np.mean([r["oracle_mean_orig_drift"] for r in city_results]))
    ...
    summary = {
        ...
        "destination_sampling": "exact_softmax_on_known_positive_support_no_sampling",
        "prod_model": {
            "architecture": "MLP(26 -> 64 -> 32 -> 1) with ReLU, bounded (min=0, max=20, clamp_min=1e-8)",
            "best_epoch": prod_epoch,
            "best_val_rmse": prod_val_rmse,
        ...
        "dg_model": {
            ...
            "max_pairs_per_batch": args.max_pairs_per_batch,
            "max_pairs_observed_in_batch": max_b_pairs,
            "epochs": 20,
        ...
        "pred_o": {
            ...
            "mean_orig_drift": mean_drift_pred_all,
        },
        "oracle_o": {
            ...
            "mean_orig_drift": mean_drift_orc_all,
        },
    ...
    print(f"DG-Predicted-O: CPC {pred_before_mean:.5f} -> {pred_after_mean:.5f} (Delta: {pred_delta_mean:+.5f}, Win: {pred_win_rate}, Mean Drift: {mean_drift_pred_all:.2%})")
    print(f"DG-Oracle-O:    CPC {oracle_before_mean:.5f} -> {oracle_after_mean:.5f} (Delta: {oracle_delta_mean:+.5f}, Win: {oracle_win_rate}, Mean Drift: {mean_drift_orc_all:.2%})")
