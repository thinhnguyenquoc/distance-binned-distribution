...
def audit_results(results_dir: Path) -> dict[str, Any]:
    ...
    for row in raw_replicates:
        ...
        _require(0 <= int(row["replicate_id"]) < EXPECTED_REPLICATES, "Replicate id outside expected range")
        expected_trips = None if row["sample"] == "inf" else int(row["sample"])
    ...
    for row in per_city:
        ...
        _require(row["fold"] in EXPECTED_FOLDS, f"Unexpected fold: {row['fold']}")
        rows_by_sample.setdefault(row["sample"], []).append(row)
        for field in ["delta_cpc", "empirical_tv", "win_rate", "harm_rate"]:
            _finite(row[field], f"{row['sample']} {row['city']} {field}")
        delta = float(row["delta_cpc"])
        _require(-1.0 <= delta <= 1.0, f"{row['sample']} {row['city']}: delta_cpc outside [-1, 1]")
        _require(0.0 <= row["empirical_tv"] <= 1.0, "TV distance outside [0, 1]")
        _require(0.0 <= row["win_rate"] <= 1.0, "Win rate outside [0, 1]")
        _require(0.0 <= row["harm_rate"] <= 1.0, "Harm rate outside [0, 1]")
        _require(row["win_rate"] + row["harm_rate"] <= 1.0 + SUMMARY_TOLERANCE, f"{row['sample']} {row['city']}: win_rate + harm_rate exceeds 1")
    ...
    _require(abs(clean_gain) > SUMMARY_TOLERANCE, "Clean gain is zero; relative-to-clean thresholds are undefined")
