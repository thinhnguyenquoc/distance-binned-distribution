...
### 4.3.2 Spatial resolution provides modest incremental gains concentrated in multi-county metropolises
*(Tiếng Việt: **4.3.2. Độ phân giải county mang lại lợi ích nhỏ và tập trung ở các thành phố đa quận**)*

Across all 50 metropolitan areas, the zero-shot baseline $M_0$ achieves a mean interzonal CPC of $0.7128$. Calibration using city-level distance distributions increases CPC to $0.7163$, corresponding to an average improvement of $+0.00354$. When conditioning distributions on origin counties ($M1_{\mathrm{county}}$), mean CPC increases marginally to $0.7165$, bringing the total gain over baseline to $+0.00368$. Consequently, the pooled incremental gain of county-level observation over city-level observation across the entire benchmark is:

$$\overline{\Delta\operatorname{CPC}}_{\mathrm{res}} = +0.00014 \quad (95\%\text{ CI: } [+0.00002, +0.00028], \text{paired Wilcoxon } p = 0.0064)$$

where the paired Wilcoxon test evaluates all 50 city-level differences using the standard zero-handling procedure for the 39 structural ties. This pooled gain is small because 39 of the 50 cities contain tracts belonging to only a single county; for these cities, $M1_{\mathrm{county}}$ is mathematically identical to $M1_{\mathrm{city}}$, producing an exact structural zero difference ($\Delta\operatorname{CPC}_{\mathrm{res},c} \equiv 0$).

Across the 11 multi-county cities, mean CPC increases from $0.7007$ under $M1_{\mathrm{city}}$ to $0.7013$ under $M1_{\mathrm{county}}$. The average incremental improvement within this heterogeneous subgroup is $+0.00063$, with 9 of the 11 cities demonstrating positive gains ($81.8\%$ win rate). The largest individual improvements occur in Kansas City ($+0.0027$), New York ($+0.0021$), and Dallas ($+0.0011$), while the remaining multi-county cities exhibit minor adjustments.

These findings indicate that refining the spatial resolution of $Y_D$ from city to county can supply supplementary predictive information, but the benefits are modest and localized. Gains concentrate in cities with multiple counties exhibiting pronounced spatial heterogeneity across origin zones. Thus, county-resolved observations improve city-wide reconstruction when significant intra-urban spatial divergence exists, but provide no structural difference when tracts share a single encompassing county.

*(Tiếng Việt: Trên toàn bộ 50 thành phố, baseline $M_0$ đạt CPC liên vùng trung bình 0.7128. Hiệu chỉnh bằng phân phối khoảng cách cấp city làm tăng CPC lên 0.7163, tương ứng với mức cải thiện $+0.0035$. Khi sử dụng các phân phối được điều kiện theo origin county, CPC tăng nhẹ lên 0.7165 và mức cải thiện so với baseline đạt $+0.0037$. Do đó, lợi ích bổ sung của county-level observation so với city-level observation trên toàn benchmark là: $\overline{\Delta\operatorname{CPC}}_{\mathrm{res}} = +0.00014$ ($95\%\text{ CI: } [+0.00002, +0.00028]$, kiểm định Wilcoxon ghép cặp $p = 0.0064$, trong đó kiểm định xử lý 39 giá trị 0 theo cấu trúc). Mức tăng pooled nhỏ vì 39 trong 50 thành phố chỉ chứa một county. Ở các thành phố này, $M1_{\mathrm{county}}$ tương đương chính xác với $M1_{\mathrm{city}}$, và chênh lệch độ phân giải quan sát được bằng 0. Trong 11 multi-county cities, CPC trung bình tăng từ 0.7007 với $M1_{\mathrm{city}}$ lên 0.7013 với $M1_{\mathrm{county}}$. Mức cải thiện bổ sung trung bình là khoảng $+0.00063$, với 9 trong 11 thành phố có chênh lệch dương ($81.8\%$). Lợi ích lớn nhất xuất hiện ở Kansas City ($+0.0027$), tiếp theo là New York ($+0.0021$) và Dallas ($+0.0011$). Những thành phố còn lại chỉ có mức thay đổi rất nhỏ. Kết quả này cho thấy việc tăng độ phân giải không gian của $Y_D$ từ city lên county có thể cung cấp thêm thông tin, nhưng lợi ích không lớn và không đồng đều. Cải thiện chủ yếu xuất hiện ở một số thành phố có nhiều county và có khả năng tồn tại khác biệt rõ hơn giữa các phân phối khoảng cách của từng vùng xuất phát. Vì vậy, kết quả hỗ trợ một kết luận có giới hạn: county-resolved observations có thể cải thiện dự báo city-wide khi thành phố chứa cấu trúc không gian nội đô đủ không đồng nhất; chúng không tạo ra lợi ích bổ sung trong trường hợp toàn bộ tract thuộc cùng một county.)*
...
The results in Table 6 demonstrate that the calibration gain transfers robustly across learned neural backbones, but is substantially attenuated for the classical gravity baseline. For the Node MLP backbone, calibration improves the mean interzonal CPC from $0.70913$ to $0.71242$, yielding $\Delta\mathrm{CPC} = +0.00329$ (95% bootstrap CI: $[+0.0025, +0.0042]$, Wilcoxon $p = 4.38 \times 10^{-11}$) with positive gains in 47 of 50 cities ($94.0\%$). In contrast, for the classical gravity model, calibration produces only a marginal, non-significant gain ($\Delta\mathrm{CPC} = +0.00084$, win rate 22/50, Wilcoxon $p = 0.3545$). This contrast suggests that $Y_D$ provides maximum utility when paired with a base model that already captures rich non-linear spatial representations from urban context features, which distance-binned mass redistribution can effectively refine.

*(Tiếng Việt: Kết quả tại Bảng 6 cho thấy mức cải thiện do hiệu chỉnh được chuyển giao ổn định qua các neural backbone có khả năng học biểu diễn, nhưng bị suy giảm rõ rệt trên mô hình trọng lực cổ điển. Với Node MLP, hiệu chỉnh cải thiện CPC trung bình $+0.00329$ ($p = 4.38 \times 10^{-11}$, thắng 47/50 thành phố). Ngược lại, trên gravity baseline cổ điển, hiệu chỉnh chỉ tạo ra mức tăng nhỏ không có ý nghĩa thống kê ($+0.00084$, thắng 22/50, $p = 0.3545$). Sự tương phản này gợi ý rằng $Y_D$ phát huy hiệu quả cao nhất khi kết hợp với một mô hình cơ sở đã có biểu diễn không gian đủ giàu để việc tái phân bổ khối lượng theo khoảng cách khai thác.)*
...
To evaluate whether the observed benefit merely reflects generic target supervision rather than the structured value of distance-aggregated constraints, we conduct a direct-OD information equivalence experiment. We compare the reconstruction gain from the $K=8$ distance-binned distribution against direct observations of positive interzonal OD pairs across sampling proportions $p \in [0.10\%, 5.0\%]$, evaluated strictly on unseen OD pairs across all 50 held-out test cities under an OD Fixed-Effect residual adapter (OD-FE).
...
This mathematical property dictates that bin scaling *cannot* alter intra-bin pair rankings. One might hypothesize that a baseline with superior intra-bin ranking fidelity ($Q_c^{\mathrm{intra}}$) would derive greater benefit from calibration. However, empirical analysis shows that intra-bin fidelity is entirely uncorrelated with calibration gain ($r = +0.046, p = 0.75$).

*(Tiếng Việt: Giới hạn toán học này cho thấy hiệu chỉnh không thể sửa thứ tự nội bin. Một giả thuyết có thể đặt ra là baseline có chất lượng xếp hạng nội bin ($Q_c^{\mathrm{intra}}$) tốt hơn sẽ hưởng lợi nhiều hơn. Tuy nhiên, phân tích thực nghiệm cho thấy chất lượng nội bin không có tương quan với mức cải thiện ($r = +0.046, p = 0.75$).)*

In sharp contrast, the baseline distance distribution mismatch $d_{\mathrm{pre}} = \mathrm{TV}(\hat{Y}_D^{(0)}, Y_D^{\mathrm{GT}})$—which measures the Total Variation error between the zero-shot baseline's implicit distance allocation and the ground truth—serves as a dominant predictor of cross-city gain heterogeneity. As reported in Table 8, $d_{\mathrm{pre}}$ exhibits a strong bivariate correlation with $\Delta\mathrm{CPC}$ (Pearson $r = +0.7995, p = 3.36 \times 10^{-12}$; Spearman $\rho = +0.7464, p = 4.92 \times 10^{-10}$). After controlling for baseline accuracy ($M_0$ CPC), number of tracts ($\log N_{\mathrm{tracts}}$), total pairs ($\log N_{\mathrm{pairs}}$), and mean geographic distance, the partial correlation remains exceptionally high ($r_{\mathrm{partial}} = +0.7951, p = 5.35 \times 10^{-12}$). In multivariate linear regression ($R^2 = 73.7\%$), $d_{\mathrm{pre}}$ accounts for the vast majority of cross-city gain variation ($\beta = +0.1487, t = +8.70, p = 4.12 \times 10^{-11}$).

*(Tiếng Việt: Ngược lại, mức độ sai lệch phân phối khoảng cách ban đầu $d_{\mathrm{pre}} = \mathrm{TV}(\hat{Y}_D^{(0)}, Y_D^{\mathrm{GT}})$ là biến dự báo tuyến tính chủ đạo giải thích tính không đồng nhất giữa các thành phố ($r = +0.7995$, partial $r = +0.7951, p = 5.35 \times 10^{-12}$). Trong mô hình hồi quy đa biến ($R^2 = 73.7\%$), $d_{\mathrm{pre}}$ giải thích phần lớn biến thiên mức cải thiện giữa các đô thị ($\beta = +0.1487, t = +8.70, p = 4.12 \times 10^{-11}$).)*

---

![Figure 5](figures/fig5_mechanistic_dpre.png)
**Figure 5 | Mechanistic diagnostic: Calibration gain increases with baseline distance misalignment.** Scatter plot of baseline distance mismatch $d_{\mathrm{pre}} = \mathrm{TV}(\hat{Y}_D^{(0)}, Y_D^{\mathrm{GT}})$ versus reconstruction gain $\Delta\mathrm{CPC}$ across all $N=50$ held-out test cities. The green line depicts the linear regression fit ($R^2 = 73.7\%$, Pearson $r = +0.7995$, $p = 3.36 \times 10^{-12}$, partial $r = +0.7951$, $p = 5.35 \times 10^{-12}$ controlling for baseline performance and network scale).
...
3. **Resolution and Fidelity Scaling**: The informational value of $Y_D$ scales continuously with observational granularity and fidelity. Increasing distance bins $K$ monotonically improves accuracy (while average gain per bin declines), refining spatial resolution preserves within-metropolitan heterogeneity in multi-county metropolises, and synthetic perturbation degrades gracefully until crossing zero at $\epsilon_{\mathrm{cross}} = 4.44\%$ Total Variation error.
4. **Model Generality and Structural Breadth**: Performance gains replicate across model initializations and neural backbones (Urban GNN and Node MLP) while attenuating on classical gravity models. Furthermore, observing 8 aggregate scalars matches the information utility of directly surveying approximately $0.20\%$ of all positive pairwise OD flows (interpolated operational crossing).
