...
*(Tiếng Việt: Sau khi hoàn thành huấn luyện và lựa chọn mô hình trên các thành phố validation, toàn bộ tham số mô hình được đóng băng vĩnh viễn. Khi suy luận trên thành phố mục tiêu $c$, mô hình chỉ tiếp nhận các dữ liệu không gian tĩnh được phép: tọa độ tâm $\mathbf{s}_{c,i}$, đặc trưng bối cảnh đô thị $\mathbf{x}_{c,i}$, và dân số tract $P_{c,i}$ trên tập hỗ trợ $\Omega_{c,\mathrm{inter}}^+$. Cường độ luồng tham chiếu $t_{c,ij}$ và phân phối khoảng cách mục tiêu $\mathbf{Y}_{D,c}$ hoàn toàn không được sử dụng ở bước này, tạo ra dự báo baseline trước hiệu chỉnh $\widehat{T}_{c,ij}^{(0,m)}$.)*
