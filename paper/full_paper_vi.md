...
Việc ước lượng cường độ luồng OD cho một thành phố mới chưa có quan sát về luồng giao thông vẫn là một thách thức. Mặc dù các mô hình zero-shot đã khai thác đặc điểm đô thị và khoảng cách địa lý để thực hiện nhiệm vụ này, giá trị bổ sung mà phân phối di chuyển theo khoảng cách có thể mang lại cho các mô hình đó vẫn chưa được làm rõ. Để làm rõ vấn đề này, nghiên cứu sử dụng phân phối di chuyển theo khoảng cách của thành phố mục tiêu để hiệu chỉnh cường độ luồng OD dự báo trên tập hỗ trợ dương liên vùng đã biết. Phân phối này được xây dựng theo thiết lập oracle từ luồng OD thực trên cùng tập hỗ trợ đánh giá và dùng để hiệu chỉnh đầu ra của mô hình zero-shot được giữ cố định.

Nghiên cứu áp dụng quy trình kiểm định chéo liên thành phố 5-fold trên 50 thành phố ở Hoa Kỳ. Kết quả thực nghiệm cho thấy hiệu chỉnh ở cấp thành phố làm CPC trung bình tăng 0.00354 (CI 95%: $[+0.0026, +0.0045]$), với 45/50 thành phố được cải thiện, và mức cải thiện giảm khi phân phối quan sát có độ phân giải thấp hơn hoặc chất lượng kém hơn. Nhìn chung, phân phối di chuyển theo khoảng cách chính xác của thành phố mục tiêu cung cấp một lượng thông tin bổ sung nhỏ nhưng nhất quán cho việc hiệu chỉnh kết quả của mô hình zero-shot.
...
Các nghiên cứu so sánh cũng cho thấy quy luật suy giảm theo khoảng cách không cố định giữa các bộ dữ liệu và bối cảnh đô thị, mà có thể thay đổi theo phương thức di chuyển, mục đích chuyến đi, mức độ đô thị hóa và điều kiện kinh tế–xã hội [@verma2025distance]. Những kết quả này cho thấy đặc điểm di chuyển theo khoảng cách phụ thuộc vào từng bối cảnh. Nghiên cứu hiện tại kế thừa ý tưởng sử dụng thông tin tổng hợp để hiệu chỉnh dự báo, nhưng sử dụng phân phối di chuyển theo các nhóm khoảng cách để hiệu chỉnh trực tiếp đầu ra của mô hình zero-shot được giữ cố định, thay vì ước lượng một tham số lực cản từ chiều dài trung bình hoặc trung vị thời gian di chuyển.
...
Hai baseline neural sử dụng cùng cấu hình huấn luyện với thuật toán tối ưu AdamW [@loshchilov2019adamw], chọn checkpoint theo CPC trên tập validation và được huấn luyện với ba seed khởi tạo ngẫu nhiên. Khi suy luận, cường độ luồng dự báo được tính bằng kỳ vọng của phân phối ZTNB:
