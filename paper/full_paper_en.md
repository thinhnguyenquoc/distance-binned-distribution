...
The study uses a graph neural network (GNN) as the main baseline, together with a multilayer perceptron (MLP) and a two-parameter Gravity model, to assess how calibration gains depend on the initial prediction model. All three baselines produce flow-intensity predictions on the known positive interzonal support and are subjected to the same calibration procedure while their model parameters remain fixed. In this design, the GNN and MLP represent nonlinear neural baselines, whereas the two-parameter Gravity model provides a simpler classical reference. This comparison evaluates whether calibration gains persist when initial predictions originate from neural models with higher accuracy within the present benchmark.
...
For each city, the improvement is computed as the CPC difference between the post-calibration prediction and the baseline, then averaged across model seeds within each city and macro-averaged across all 50 cities. The city is the statistical unit ($N=50$); results across the three model seeds are averaged prior to bootstrap resampling and the Wilcoxon test.

The 95% confidence interval for the mean cross-city CPC difference is estimated using a city-level paired nonparametric bootstrap, stratified by fold [@efron1993bootstrap]. These confidence intervals are computed conditional on the fixed fold splits and trained models, and do not capture variation from re-splitting folds or retraining models. Paired differences are evaluated using a two-sided Wilcoxon signed-rank test [@wilcoxon1945ranking], which assesses the signs and ranks of the paired differences and is not interpreted as a direct test of the mean difference. Thus, the bootstrap confidence interval and the Wilcoxon test are reported in tandem as complementary descriptions. The proportion of cities with $\Delta\mathrm{CPC} > 0$ is reported as an additional descriptive statistic. Corresponding sensitivity and robustness analyses are presented in Section 4.
...
Across evaluated oracle configurations, increasing the number of distance intervals improves the average gain. When distributions are perturbed by the evaluated noise mechanism, this benefit diminishes and can reverse into CPC degradation.
...
The calibration structure also clearly limits the type of bias that $Y_D$ can address. The signal provides information about how total flow volume should be distributed across distance bands, but it provides no additional information for distinguishing OD pairs within the same distance interval. Furthermore, total predicted flow volume is strictly preserved, so calibration cannot rectify overall scale errors in the baseline. Therefore, the operator is primarily able to correct between-bin biases, where the baseline has allocated the wrong amount of mass across distance bands; within-bin allocations and overall volume errors remain determined by the baseline. The analysis in Section 4.5 is consistent with this calibration role in adjusting mass allocation across distance intervals. However, because the initial distribution bias is computed using the oracle distribution, this analysis does not provide an independent decision rule for when calibration should be applied. The value of the target distribution depends on the initial predictions it calibrates. Gains persist across two neural baselines with higher initial CPC, while remaining smaller and less consistent for Gravity. This pattern reduces the likelihood that the primary findings are merely an artifact of selecting an inaccurate baseline. Nonetheless, because GNN and MLP share input representations and the pairwise decoder, conclusions remain bounded to the evaluated architectures.
...
$$
   ...
   The TV error level measures the difference between two share distributions; it does not represent trip misclassification rate or per-OD-pair error. The experiment uses $K=8$, model seeds $\{1,10,100\}$, five folds, 50 cities, one replicate at $\epsilon=0$, and 1,000 replicates at each positive level. Replicates are averaged before seeds, and seeds before city-level inference. Bootstrap intervals reflect variation among evaluated cities conditional on the fixed folds, checkpoints, and pipeline; they do not include variation from retraining models or re-splitting folds.

2. **Placebo controls and intervention dose matching (Dose-Matched Controls)**:

   To control the intervention magnitude by the RMS of the centered log-ratio vector, control distributions are scaled to match the RMS value of the target calibration signal. For each evaluated city, let $\widehat{Y}^{(0)}$ denote the distance distribution predicted by the zero-shot baseline $M_0$ over active intervals ($b \in \mathcal A_c$). The log-ratio vector of the target distribution and its centered root-mean-square intervention magnitude $D_T$ ($D_T = \|\tilde{\mathbf{r}}_T\|_2 / \sqrt{K_{\mathrm{act}}}$) are given by:
...
$$
   ...
   - **Training-city donor control (Wrong-City Donors, Dose-Matched)**: For each random donor draw from training cities within the same fold ($B_{\mathrm{draw}} = 1,000$), let $Y_D^{\mathrm{donor}}$ be the donor distribution. Prior to computing the log-ratio, the procedure extracts the donor shares across the active intervals of the target city. If any share falls below $\delta = 10^{-12}$, the shares are lower-bounded at $\delta$ and then re-normalized over this set. The baseline predicted share used in the log-ratio is also lower-bounded at $\delta$. The log-ratio vector is subsequently centered across the active intervals before computing the RMS. The initial log-ratio and donor intervention magnitude $D_D$ are computed as:
...
$$
   ...
   - **Fold training-mean donor control (Training-Mean Donor, Dose-Matched)**: The control distribution is computed as the unweighted arithmetic mean of the normalized distance distributions across the 35 training cities in the corresponding fold, giving equal weight to each city. The training-mean distribution is processed using the same log-ratio function and numerical stabilization rules as the donor control. In the evaluated folds, the mean distribution prior to the lower-bounding step has strictly positive mass across all active intervals of the target city. Its log-ratio and initial intervention magnitude $D_M$ are:
...
$$
   ...
   - **Permuted calibration log-ratio control**: To test the role of the correspondence between the calibration signal and distance intervals, the components of the centered target log-ratio vector are permuted across active intervals: $\tilde{r}_{P,b} = \tilde{r}_{T,\pi(b)}$, where $\pi$ is a permutation over $\mathcal A_c$ distinct from the identity ($\pi \neq \mathrm{id}$). In the main configuration $K=8$, the procedure first generates the set of non-identity index permutations over active intervals. If this set contains at most 1,000 elements, all permutations are evaluated; if it contains more than 1,000 elements, exactly 1,000 permutations are randomly sampled without replacement. The control performance is computed as the mean CPC across the evaluated permutations. In the evaluated benchmark, 3 cities use the full set of valid non-identity permutations, with counts of 719, 119, and 119, respectively; the remaining 47 cities evaluate 1,000 permutations each. Permutation preserves the Euclidean and RMS norms of the centered log-ratio vector ($\|\tilde{\mathbf{r}}_P\|_2 = \|\tilde{\mathbf{r}}_T\|_2 = \sqrt{K_{\mathrm{act}}} D_T$), while randomizing the correspondence between the components of this vector and the distance intervals. The permuted distribution is then reconstructed via:
