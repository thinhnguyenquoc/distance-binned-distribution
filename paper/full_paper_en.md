...
From a methodological baseline perspective, an intuitive question arising from classical frameworks such as Hyman [@hyman1969calibration] and Merlin [@merlin2020medians] is whether $Y_D$ could simply be used to recalibrate a single parametric distance-decay exponent $\alpha$ in a standard gravity model. Here, multi-bin calibration ($K=8$) provides piecewise nonparametric flexibility across the entire distance spectrum, fundamentally surpassing a rigid parametric tilt of an exponential or power-law decay curve. Indeed, as shown in Table 5, even when equipped with this flexible binned calibration operator, the Two-Parameter Gravity model achieves a CPC gain of only $+0.00084$ (improving in only 22/50 cities), markedly lower than the gains attained by calibrated neural architectures ($+0.00354$ on Urban GNN and $+0.00329$ on Node MLP). This demonstrates that the primary utility of target-domain distance calibration is realized when combined with expressive, deep urban representations rather than merely refitting a univariate decay parameter.
