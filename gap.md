...
## novelty
- Định lượng marginal reconstructive value của một target-city distance-binned mobility distribution đối với một mô hình Physics-Informed GNN đã đóng băng (Gravity-Informed Urban GNN). Bằng cách thiết lập baseline là một mô hình GNN cực mạnh đã tích hợp sẵn định luật hấp dẫn (gravity), việc bổ sung Y_D ở bước inference chứng minh được giá trị độc lập của phân phối khoảng cách đích mà một mô hình không gian thuần túy không thể tự nội suy.
## Gap
- Mặc dù thông tin về độ dài chuyến đi đã được sử dụng để hiệu chỉnh các mô hình tương tác không gian truyền thống, và các kiến trúc GNN (Graph Neural Networks) gần đây (đặc biệt là Physics-Informed GNN kết hợp Gravity) có thể dự báo OD zero-shot rất chính xác từ bối cảnh đô thị và khoảng cách địa lý, vẫn chưa rõ liệu việc bổ sung phân phối di chuyển theo các khoảng khoảng cách (Y_D) của thành phố mục tiêu tại thời điểm suy luận có đem lại cải thiện (marginal gain) vượt trên một baseline zero-shot đã đóng băng và rất mạnh hay không.
## Narrative
- Prior studies demonstrate that mean, median, and full trip-length distributions can be used to calibrate the distance-decay component of classical spatial-interaction models. Separately, recent cross-city deep learning models, such as Physics-Informed GNNs (combining Graph Neural Networks with Gravity principles), can generate highly accurate OD flows zero-shot from urban context and pairwise distance. However, these two research streams have not established whether a coarse target-city distance-binned distribution provides non-redundant, destination-resolved information when applied to calibrate an otherwise frozen, state-of-the-art Gravity-Informed GNN prediction. This unresolved marginal value over a strong physics-informed baseline constitutes the research gap.
